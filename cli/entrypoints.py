...
def _claude_child_env(
    ...
    # Default model overrides for latest Claude Code CLI (supports Opus 4.8+)
    # These route through the proxy which maps them to the configured provider.
    env.setdefault("ANTHROPIC_DEFAULT_OPUS_MODEL", "claude-opus-4-8")
    env.setdefault("ANTHROPIC_DEFAULT_SONNET_MODEL", "claude-sonnet-4-6")
    env.setdefault("ANTHROPIC_DEFAULT_HAIKU_MODEL", "claude-haiku-4-5-20251001")
